...
class AttributeField(Field):
    ...
    def to_rst(self, indent=4):
        ...
        type_str = '' if is_empty(_type) else annotation.format(indent * ' ',
                                                                _type)
...
class ArgumentField(Field):
    ...
    def to_rst(self, indent=4):
        ...
        indent_str = ' ' * indent
        param_str = '{0}:param {1}: {2}'.format(indent_str, name,
                                                self.desc[0].strip())
...
def max_desc_length(fields):
    ...
    """
    return max([
        len(' '.join([line.strip() for line in field[2]])) for field in fields
    ])
