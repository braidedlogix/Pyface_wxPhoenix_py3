...
class DockImages(HasPrivateTraits):
    ...
    def init(self):
        ...
        self._splitter_images = [
            ...
            for name in [
                'sv_left', 'sv_middle', 'sv_right', 'sh_top', 'sh_middle',
                'sh_bottom'
            ]
        ]
...
class DockItem(HasPrivateTraits):
    ...
    def mouse_up(self, event):
        ...
        # Handle the completion of a dragging operation:
            ...
            # Check to see if the user is attempting to copy the control:
                ...
                owner = window.owner
                control = owner.handler.dock_control_for(*(owner.handler_args +
                                                           (window, control)))
    ...
    def mouse_move(self, event):
        ...
        region = self.parent
        if ((not event.ControlDown()) and (dock_info.region is region) and
        ...
        if ((dock_info.kind == DOCK_EXPORT) and (self.export != '') and
    ...
    def _close_bounds(self):
        ...
        if self.closeable and self._is_tab:
            ...
            return (x + dx + ox - slice.xright - tc.right - CloseTabSize,
                    y + oy + (
                        (dy + slice.xtop + tc.top - slice.xbottom - tc.bottom -
                         text_dy) // 2) + 3, CloseTabSize, CloseTabSize)
    ...
    def pre_drag(self, object, tag=0):
        ...
        """
        if (self.visible and (self.feature_mode != FEATURE_NONE) and
    ...
    def collapse(self, forward):
        ...
        if is_horizontal:
            if state != SPLIT_HMIDDLE:
                if ((y == self.bounds[1]) or (y < iy1) or (
                    (y + dy) > (iy2 + idy2))):
                    y = (iy1 + iy2 + idy2 - dy) // 2
            else:
            ...
            if ((x == self.bounds[0]) or (x < ix1) or (
                (x + dx) > (ix2 + idx2))):
                x = (ix1 + ix2 + idx2 - dx) // 2
        else:
            self._last_bounds = self.bounds
...
class DockControl(DockItem):
    ...
    def check_features(self):
        ...
        mode = self.feature_mode
        n = len(features)
        if ((self.num_features < n) and (self.control is not None) and
...
class DockRegion(DockGroup):
    ...
    def calc_min(self, use_size=False):
        ...
        if self.is_notebook:
            ...
            tdx = max(tdx, tab_dx) + (tis.xleft + tis.xright + tc.left +
                                      tc.right)
            tdy += (theme.tab_active.image_slice.dy + tis.xtop + tis.xbottom +
                    tc.top + tc.bottom)
        elif len(contents) > 0:
            item = contents[0]
    ...
    def recalc_sizes(self, x, y, dx, dy):
        ...
        if self.is_notebook:
            ...
            if tx > xr:
                ...
                self._tab_clip_bounds = (tx0, y,
                                         cdx - DockImages._tab_scroller_dx, th)
            else:
    ...
    def object_at(self, x, y):
        ...
        if (self._visible is not False) and self.is_at(x, y):
            if self.is_notebook and (self.tab_scroll_index >= 0):
                ...
                if self.is_at(x, y,
                              (cx + cdx, cy + 2, DockImages._tab_scroller_dx,
    ...
    def scroll(self, type, left_tab=0):
        ...
        if left_tab != self.left_tab:
            ...
            self.tab_scroll_index = (
                (left_tab < self.max_tab) + (2 * (left_tab > 0))) - 1
...
class DockSection(DockGroup):
    ...
    def update_splitter(self, splitter, window):
        ...
        window.RefreshRect(
            wx.Rect(ix1 - dx, iy1 - dy, ix2 + idx2 - ix1 + 2 * dx, iy2 + idy2 -
                    iy1 + 2 * dy))
            ...
            if (kind == DOCK_TAB) or (kind == DOCK_BAR):
                ...
                if ((section.is_row and (
                    (kind == DOCK_TOP) or (kind == DOCK_BOTTOM))) or (
                        (not section.is_row) and ((kind == DOCK_LEFT) or
                                                  (kind == DOCK_RIGHT)))):
                    if len(section.contents) > 0:
                        sizer._contents = section = DockSection(
                            is_row=not section.is_row).set(contents=[section])
                if len(section.contents) > 0:
                    i = 0
