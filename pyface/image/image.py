...
import sys
from os import (environ, listdir, remove, stat, makedirs, rename, access, R_OK,
                W_OK, X_OK)
from os.path import (join, isdir, isfile, splitext, abspath, dirname, basename,
                     exists)
...
from traits.api import (HasPrivateTraits, Property, Str, Int, List, Dict, File,
                        Instance, Bool, Undefined, TraitError, Float, Any,
                        cached_property)
...
class ImageInfo(HasPrivateTraits):
    ...
    def _get_image_info_code(self):
        ...
        data.update(('c' + name, getattr(self.content, name))
                    for name in sides)
