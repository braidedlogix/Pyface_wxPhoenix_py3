...
class MainWindowLayout(HasTraits):
    ...
    def _add_dock_panes(self, window):
        ...
        for dock_pane in self.state.dock_panes:
            ...
            if not info.IsOk():
                logger.debug("_add_dock_panes: managing pane %s" %
                             dock_pane.pane_name)
                dock_pane.add_to_manager()
            else:
                logger.debug("_add_dock_panes: arleady managed pane: %s" %
                             dock_pane.pane_name)
    ...
    def _set_layout_from_aui(self, layout, window):
        ...
        self._add_dock_panes(window)
        logger.debug("_set_layout_from_aui: restoring perspective %s" %
                     layout.perspective)
