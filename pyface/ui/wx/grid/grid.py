...
class Grid(Widget):
    ...
    def __init__(self, parent, **traits):
        ...
        otc(self._on_default_cell_text_color_changed,
    ...
    def dispose(self):
        ...
        otc(self._on_default_label_font_changed,
            'default_label_font',
            remove=True)
        ...
        otc(self._on_default_label_text_color_changed,
            'default_label_text_color',
            remove=True)
        otc(self._on_selection_bg_color_changed,
            'selection_bg_color',
            remove=True)
        otc(self._on_selection_text_color_changed,
            'selection_text_color',
            remove=True)
        otc(self._on_default_cell_font_changed,
            'default_cell_font',
            remove=True)
        otc(self._on_default_cell_text_color_changed,
            'default_cell_text_color',
            remove=True)
        otc(self._on_default_cell_bg_color_changed,
        ...
        otc(self._on_column_label_height_changed,
            'column_label_height',
            remove=True)
        otc(self._on_row_label_width_changed, 'row_label_width', remove=True)
        otc(self._on_show_column_headers_changed,
            'show_column_headers',
            remove=True)
    ...
    def _on_model_structure_changed(self, *arg, **kw):
        ...
        if delta > 0:
            ...
            # Rows were deleted:
            msg = GridTableMessage(self._grid_table_base,
                                   GRIDTABLE_NOTIFY_ROWS_DELETED, row_count,
                                   -delta)
            grid.ProcessTableMessage(msg)
            should_autosize = True
        ...
        if delta > 0:
            ...
            msg = GridTableMessage(self._grid_table_base,
                                   GRIDTABLE_NOTIFY_COLS_DELETED, col_count,
                                   -delta)
            grid.ProcessTableMessage(msg)
            should_autosize = True
    ...
    def _on_range_select(self, evt):
        if evt.Selecting():
            if (self.selection_mode == 'cell') and evt.ControlDown():
                self._grid.SelectBlock(evt.GetTopRow(),
                                       evt.GetLeftCol(),
                                       evt.GetBottomRow(),
                                       evt.GetRightCol(), True)
    ...
    def _on_col_move(self, evt):
        ...
        if self.model._move_column(frm, to):
            ...
            grid.ProcessTableMessage(
                GridTableMessage(self._grid_table_base,
